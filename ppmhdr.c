...
bdf2c_fontpic_draw_metric ()
{
    ...
    // description
    sprintf (buf1, "generated by ppmhdr. Copyright 2015 Yunhui Fu <yhfudev@gmail.com>");
    ppm_cavas_drawstring (pcavas, PPM_OFF_X + 1, PPM_OFF_X - FNT_HEIGHT - 5, color_shift, color_background, buf1);
    sprintf (buf1, "Char Sheet for %s, char width=%lu,height=%lu", fntname, char_wx, char_hy);
    ppm_cavas_drawstring (pcavas, PPM_OFF_X + 1, PPM_OFF_X - FNT_HEIGHT - 1 - FNT_HEIGHT - 5, color_shift, color_background, buf1);
    // draw line:
    ...
    for (i = 0; i < (char_wx + 2*PPM_FNT_BORDER) * pic_xnum; i += (char_wx + 2*PPM_FNT_BORDER)) {
        ...
        sprintf (buf1, "%02X", cnt);
        cnt ++;
        ppm_cavas_drawstring (pcavas, char_wx / 2 + PPM_FNT_BORDER + i + PPM_OFF_X - FNT_WIDTH, 4, color_shift, color_background, buf1);
    }
    ...
    for (i = 0; i < (char_wx + 2*PPM_FNT_BORDER) * pic_xnum; i += (char_wx + 2*PPM_FNT_BORDER)) {
        ...
        sprintf (buf1, "%02X", cnt);
        cnt ++;
        ppm_cavas_drawstring (pcavas, char_wx / 2 + PPM_FNT_BORDER + i + PPM_OFF_X - FNT_WIDTH, PPM_OFF_Y - 4 - FNT_HEIGHT - 1, color_shift, color_background, buf1);
    }
    ...
    for (i = 0; i < (char_hy + 2*PPM_FNT_BORDER) * pic_ynum; i += (char_hy + 2*PPM_FNT_BORDER)) {
        ...
        sprintf (buf1, "%02X", cnt);
        cnt ++;
        ppm_cavas_drawstring (pcavas, 4, char_hy / 2 + PPM_FNT_BORDER + i + PPM_OFF_Y - FNT_HEIGHT/2 - 1, color_shift, color_background, buf1);
    }
    ...
    for (i = 0; i < (char_hy + 2*PPM_FNT_BORDER) * pic_ynum; i += (char_hy + 2*PPM_FNT_BORDER)) {
        ...
        sprintf (buf1, "%02X", cnt);
        cnt ++;
        ppm_cavas_drawstring (pcavas, PPM_OFF_X - 3 - (FNT_WIDTH + 1) * 2, char_hy / 2 + PPM_FNT_BORDER + i + PPM_OFF_Y - FNT_HEIGHT/2 - 1, color_shift, color_background, buf1);
...
test_set_hz(ppm_cavas_t *pcavas, size_t offx, size_t offy)
{
    ...
    sprintf (buf1, "ABCabc0123: %02X", 391);
